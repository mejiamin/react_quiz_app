...
## Всего у нас будет 3 урока:

- Урок 1: Архитектура, структура папок и настройка React Router. Подготовка к работе.

- Урок 2: Самый сок логики. Импорт JSON, генерация случайных вариантов ответов (с проверкой, чтобы правильный ответ не дублировался в неправильных), обработка кликов с подсветкой (зеленый/красный) и переход к следующему вопросу.
  
- Урок 3: Передача финального счета на страницу ResultsPage, вывод результатов, кнопка «Игры заново» (сброс состояния) и финальная полировка стилей CSS Modules.
